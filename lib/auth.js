...
// ✅ Nouvelle version — App Router (cookies() sans req)
export async function getTokenFromAppRouter(expectedRole = null) {
  try {
    await dbConnect();
    ...
    const role = volunteer.role;
    if (expectedRole === "supervisor" && role !== "supervisor") throw new Error("Rôle non autorisé");
    if (expectedRole === "volunteer" && !["volunteer", "supervisor"].includes(role)) throw new Error("Rôle non autorisé");

    return { ...volunteer.toObject(), role };
  } catch (err) {
    console.error("❌ Erreur dans getTokenFromAppRouter :", err.message);
    return null;
  }
}
